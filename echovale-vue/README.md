...
*   **验证码系统**：集成 TianAiCaptcha 滑块验证，支持验证失败自动刷新和健壮的生命周期管理。
*   **交互反馈**：集成 Element Plus 的消息提示系统，为登录等关键操作提供即时反馈。
...
*   **Emits**:
    *   `success` (token: string): 验证成功时触发，传递验证令牌。
    *   `update:modelValue` (value: boolean): 内部关闭对话框时更新状态。
*   **特性**:
    *   **自动刷新**：验证失败（如 4001 错误）时自动重新加载题目。
    *   **生命周期安全**：对话框关闭或重新初始化时会自动销毁旧实例，防止 DOM 冲突。
...
2.  **验证码初始化**：
    *   使用 `nextTick` 确保在 `v-dialog` 过渡动画完成后再执行 DOM 绑定，避免 `dom不存在` 错误。

## 后续优化建议

*   **懒加载 (Lazy Loading)**：针对 `CaptchaDialog` 等重型组件，可考虑使用异步组件加载以优化首屏。
*   **虚拟列表**：若未来涉及大量音乐列表，建议引入虚拟滚动技术。
*   **Teleport**：将全局弹窗挂载至 `body` 根节点，避免复杂的层级 z-index 冲突。
